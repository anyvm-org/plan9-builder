...
reply(Fcall *r, Fcall *t, char *err)
{
	...
	if(err != nil) {
		...
		t->ename = err;
		/*
		 * anyvm: strip the kernel's path decoration ("error: 'path'")
		 * so remote clients that map error strings to error numbers
		 * (Linux v9fs p9_errstr2errno) recognize the bare error text.
		 */
		if((p = strstr(t->ename, ": '")) != nil)
			*p = '\0';
	}
	else
		t->type = r->type + 1;
